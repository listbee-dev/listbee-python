...
class Listings:
    ...
    def create_and_publish(
        ...
        *,
        ...
        """Create a listing, optionally set deliverables, and publish — all in one call.

        This is the recommended way to create a live listing. Equivalent to calling
        ``create()``, then ``set_deliverables()`` (if deliverables provided), then ``publish()``.

        Args:
            name: Product name.
            price: Price in smallest currency unit (e.g. 2900 = $29.00).
            deliverables: Optional array of deliverable dicts. Each has ``type``
                ('file', 'url', 'text') and ``token`` (files) or ``value`` (url/text).
            fulfillment: Fulfillment mode — "managed" or "external".
            checkout_schema: Custom fields collected at checkout. Max 10 fields.
            description: Longer product description, plain text.
            tagline: Short line shown below the product name.
            ...
            timeout: Timeout for the create call (default 120s for cover processing).
        ...
        if deliverables:
            self.set_deliverables(listing.id, deliverables=deliverables)
        return self.publish(listing.id)

    def upload_and_set_deliverable(
        self,
        listing_id: str,
        *,
        ...
        """Upload a file and set it as the listing's deliverable in one call.

        Equivalent to ``client.files.upload(file=file)`` then
        ``client.listings.set_deliverables(listing_id, deliverables=[{"type": "file", "token": file_token}])``.
...
class AsyncListings:
    ...
    async def create_and_publish(
        ...
        *,
        ...
        """Create a listing, optionally set deliverables, and publish — all in one call (async).
        ...
        if deliverables:
            await self.set_deliverables(listing.id, deliverables=deliverables)
        return await self.publish(listing.id)

    async def upload_and_set_deliverable(
        self,
        listing_id: str,
        *,
        file: tuple[str, bytes, str],
        ...
        """Upload a file and set it as the listing's deliverable in one call (async).
        ...
        return await self.set_deliverables(
            listing_id,
            deliverables=[{"type": "file", "token": uploaded.id}],
        )

...
FAILED_EVENT_JSON = {**WEBHOOK_EVENT_JSON, "id": "evt_failed01", "status": "failed", "delivered_at": None}
FAILED_EVENT_2_JSON = {**WEBHOOK_EVENT_JSON, "id": "evt_failed02", "status": "failed", "delivered_at": None}
RETRIED_EVENT_JSON = {**WEBHOOK_EVENT_JSON, "id": "evt_failed01", "status": "delivered"}

FAILED_EVENTS_PAGE = {
    "object": "list",
    "data": [FAILED_EVENT_JSON, FAILED_EVENT_2_JSON],
    "has_more": False,
    "total_count": 2,
    "cursor": None,
}
...
class TestRetryFailedEvents:
    def test_retries_all_failed_events(self, webhooks):
        retried_1 = {**RETRIED_EVENT_JSON, "id": "evt_failed01"}
        retried_2 = {**RETRIED_EVENT_JSON, "id": "evt_failed02"}
        with respx.mock(base_url="https://api.listbee.so") as mock:
            mock.get("/v1/webhooks/wh_3mK8nP2qR5tW7xY1/events").mock(
                return_value=httpx.Response(200, json=FAILED_EVENTS_PAGE)
            )
            mock.post("/v1/webhooks/wh_3mK8nP2qR5tW7xY1/events/evt_failed01/retry").mock(
                return_value=httpx.Response(200, json=retried_1)
            )
            mock.post("/v1/webhooks/wh_3mK8nP2qR5tW7xY1/events/evt_failed02/retry").mock(
                return_value=httpx.Response(200, json=retried_2)
            )
            results = webhooks.retry_failed_events("wh_3mK8nP2qR5tW7xY1")
        assert len(results) == 2
        assert all(isinstance(r, WebhookEventResponse) for r in results)

    def test_returns_empty_list_when_no_failed_events(self, webhooks):
        with respx.mock(base_url="https://api.listbee.so") as mock:
            mock.get("/v1/webhooks/wh_3mK8nP2qR5tW7xY1/events").mock(
                return_value=httpx.Response(200, json=EMPTY_EVENTS_PAGE)
            )
            results = webhooks.retry_failed_events("wh_3mK8nP2qR5tW7xY1")
        assert results == []

    def test_filters_by_failed_status(self, webhooks):
        with respx.mock(base_url="https://api.listbee.so") as mock:
            route = mock.get("/v1/webhooks/wh_abc/events").mock(
                return_value=httpx.Response(200, json=EMPTY_EVENTS_PAGE)
            )
            webhooks.retry_failed_events("wh_abc")
        assert "status=failed" in str(route.calls[0].request.url)

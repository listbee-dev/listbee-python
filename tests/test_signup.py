...
class TestSignupCreate:
    def test_create_returns_signup_response(self):
        ...
        with respx.mock(base_url="https://api.listbee.so") as mock:
            mock.post("/v1/account").mock(return_value=httpx.Response(200, json=SIGNUP_RESPONSE))
    ...
    def test_create_sends_no_auth_header(self):
        client = ListBee(api_key=None, base_url="https://api.listbee.so")
        with respx.mock(base_url="https://api.listbee.so") as mock:
            route = mock.post("/v1/account").mock(return_value=httpx.Response(200, json=SIGNUP_RESPONSE))
...
class TestSignupVerify:
    def test_verify_returns_verify_response(self):
        client = ListBee(api_key=None, base_url="https://api.listbee.so")
        with respx.mock(base_url="https://api.listbee.so") as mock:
            mock.post("/v1/account/verify/otp").mock(return_value=httpx.Response(200, json=VERIFY_RESPONSE))
    ...
    def test_verify_sends_no_auth_header(self):
        client = ListBee(api_key=None, base_url="https://api.listbee.so")
        with respx.mock(base_url="https://api.listbee.so") as mock:
            route = mock.post("/v1/account/verify/otp").mock(return_value=httpx.Response(200, json=VERIFY_RESPONSE))
